...
//AICI AM MAI MODIFICAT                 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        // daca nu pot cuceri nimic nou, stau sau dau puterea
                        // adica voi da puterea unui vecin cu productie mica
                        else if(prodMax == -1 && myNeighbours < 4) {
                          int giveToMinProd = -1;
                            for(int i = 0 ; i < myNeighbours; i++) {
                              neighbour = presentMap.getSite(presentMap.getLocation(location, i));
                              // aleg vecinul cu productia minima
                              if(neighbour.production < giveToMinProd) {
                                move = i;
                                giveToMinProd = neighbour.production;
                              }
                            }
                            if(giveToMinProd == -1) {
                              move = 0;
...
// AICI ERA DOAR LINIA CU RANDOM
                                ...
                                float distantaMin = 101;
                                float unghi = 0;
                                hlt::Location locatiaMin;
                                poz = -1;

                                for(unsigned short m = 0; m < presentMap.height; m++) {
                                    for(unsigned short n = 0; n < presentMap.width; n++) {
                                        auxSite = presentMap.getSite({ n, m });
                                        auxLocation.x = n;
                                        auxLocation.y = m;
